 ...
 * @LastEditTime: 2025-05-07 09:10:52
...
void FOC_Control_Init(void) {
    ...
    // 初始化PID参数 (需要根据实际电机调整)
    PID_Init(&foc_ctrl.current_q_pid, 0.5f, 50.0f, 0.0f, 12.0f, 1.0f); // q轴电流环
    PID_Init(&foc_ctrl.current_d_pid, 0.5f, 50.0f, 0.0f, 12.0f, 1.0f); // d轴电流环
    PID_Init(&foc_ctrl.velocity_pid, 0.1f, 0.5f, 0.001f, 5.0f, 2.0f);  // 速度环
    PID_Init(&foc_ctrl.position_pid, 5.0f, 0.1f, 0.05f, 10.0f, 5.0f);  // 位置环
